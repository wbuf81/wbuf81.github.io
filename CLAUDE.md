...
- `status` — the week's average weight vs the week before as the hero (the same never-a-single-weigh-in rule as the page's lead card; the Sunday weigh-in is named for what it is in the footer), with the cut below it: a ladder of blocks, one per pound between the start weight and the goal, plus rate, projection and the weigh-in trend. The bottom is a three-row table — this week and the two before, oldest first, older rows faded — with each week's averages: weight (+delta), calories, protein, steps, lifts · cardio. It was one row of just the newest week, then briefly weight-only; Wes wanted the full averages *and* the trajectory. The lifts·cardio separator is deliberately wide-spaced so "5 · 3" cannot scan as 5.3. Copy is deliberately dry — facts, no narration; Wes flagged the explanatory captions as reading AI-written.
