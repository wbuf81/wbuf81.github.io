...
- **Card copy says what a thing does and what it runs on — never repo statistics.** No line counts, test counts, commit counts or coverage: they mean nothing to a visitor and go stale as soon as the repo moves. Wes asked for this explicitly.
- **Every project on the page is the same tile** — 3-up `agent-grid`, image band, title, badge, description, links. The personal projects were full-width rows until each had a picture; two card shapes on one page is what read as disjointed. Don't reintroduce a one-off layout for a single card. A tile's band shows `shot` (a screenshot), else the `hardware.image` (the board), so no tile in a row is left blank; when the band is already the board, the link below drops its duplicate thumbnail. Screenshots fill the band (`is-shot`) and share the mascots' hover — grayscale at rest, colour and a slight zoom on hover — so the whole page reads monochrome until you point at something. They take plain `grayscale(100%)` without the brightness/contrast lift the photographs get, which washes a dark UI out. A board product shot is `contain`ed on white so the device isn't cropped out of frame, and a card that names a board always shows its thumbnail in the links row, even when the band above is the same photo — the links should look identical on every tile. Long personal titles wrap, so their badge stacks under the title (`is-stacked`) rather than wedging into a wrapped line.
- A personal card may carry an optional `hardware: { label, href }` — the board it runs on, linked to the vendor's product page (Waveshare, M5Stack). Cards are plain `div`s, not one big anchor, because a card can hold two destinations and a private repo holds only the board link. **Verify a product URL returns 200 before shipping it** — vendor shops retire SKUs and a dead link on the front page is worse than no link. `hardware.image` is a small board photo drawn beside the link, stored in `public/boards/` and **hosted locally, never hotlinked**. Those are vendor photographs — `public/boards/SOURCES.md` records where each came from, and they're the placeholder until there are Wes's own photos of the boards. A card may also carry `shot` — a screenshot of the thing itself, in `public/projects/`, drawn above the links in the same last column (`card-aside` holds both; two loose grid children would break the row). Board photos carry the same grayscale filter as the mascots (colour returns on hover) so a catalog shot doesn't sit in full colour beside grayscale cards; pick a shot free of marketing text and demo screen art, and crop to the device.
...
- **Weeks are named for the Sunday they end on** ("Week ending Sep 20"), not the Monday they start. The week is posted after it closes, so that is how Wes refers to it (Sep 2026). The label is built once in `groupIntoWeeks` and every consumer — lead card, goals panel, weekly table, consistency grid, chart axes — reads that one field, so don't relabel a single component back to the start date.
