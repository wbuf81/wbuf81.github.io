# Board photographs

Product photographs of the boards these projects run on, shown as small
thumbnails beside the link to each vendor's own product page.

| File | Board | Source |
|---|---|---|
| `m5stack-core-basic.jpg` | M5Stack Core Basic v2.7 | M5Stack — `static-cdn.m5stack.com/resource/docs/products/core/basic/basic_01.webp`, cropped to the device |
| `waveshare-knob-touch-1.8.jpg` | Waveshare ESP32-S3-Knob-Touch-LCD-1.8 | Waveshare wiki — `ESP32-S3-Knob-Touch-LCD-1.8-6.png`, one panel cropped |

These images belong to their respective vendors and are used here to identify
the hardware, each linked to the vendor's product page. Replace them with our
own photographs of the boards when there are some.

Project screenshots live in `public/projects/` and all come from the projects'
own repositories:

| File | From |
|---|---|
| `streamdeck-neo.jpg` | `streamdeckneoclaude` — `docs/images/page-claude.png`, a real rendered frame |
| `omarchy-workspace-labels.jpg` | `omarchy-workspace-labels` — `docs/preview.png` |
| `m5-spotify-deck.jpg` | `m5-spotify-deck` — `docs/screenshots/snes.png` |
| `knob-cover-light.jpg` | `esp32knobtouch` — `assets/screens/cover-light.png` |
| `daisy-status-bar.jpg` | `daisy-claude-status-bar` — `art/showcase/menubar-states.png`, dark column only |
